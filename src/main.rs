...
// - TCP Listener = Library's main entrance
                                        ...
                                        match command.as_str() {
                                            ...
                                            "PSYNC" => {
                                                if args.len() >= 3 && args[1] == "?" && args[2] == "-1" {
                                                    ...
                                                    // The stream is moved to the replicas list, so we break out of
                                                    // the client handler loop to avoid racing on reads
                                                    ...
                                                    if let (Ok(numreplicas), Ok(timeout_ms)) = (args[1].parse::<u32>(), args[2].parse::<u32>()) {
                                                        ...
                                                        if numreplicas == 0 {
                                                            ...
                                                            if current_offset == 0 {
                                                                ...
                                                                let active_replicas: Vec<TcpStream> = replicas_guard.drain(..).collect();
                                                                drop(replicas_guard);

                                                                // Send GETACK to all replicas first
                                                                let mut replicas_to_read: Vec<TcpStream> = Vec::new();
                                                                for mut replica in active_replicas {
                                                                    if replica.write_all(getack_command).is_ok() {
                                                                        replicas_to_read.push(replica);
                                                                    }
                                                                }
                                                                ...
                                                                // Subtract elapsed time plus a small buffer for thread spawn/join overhead
                                                                let overhead_buffer = Duration::from_millis(20);
                                                                ...
                                                                // Restore replicas
                                                                let mut replicas_guard = replicas_clone.lock().unwrap();
                                                                replicas_guard.extend(returned_replicas);
                                                                drop(replicas_guard);
